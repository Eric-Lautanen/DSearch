...
pub fn resolve_keyword_to_urls(providers: &[SearchProvider], keyword: &str) -> Vec<(String, String)> {
    ...
    for provider in providers {
        ...
        let separator = if provider.endpoint.contains('?') { "&" } else { "?" };
        let url = format!("{}{}q={}", provider.endpoint, separator, keyword.replace(' ', "+"));
...
pub fn remove_provider(data_dir: &Path, name: &str) -> Result<bool, String> {
    ...
    let contents = std::fs::read_to_string(&path)
        .map_err(|e| format!("read: {}", e))?;
    let mut config = toml::from_str::<SearchProvidersToml>(&contents)
