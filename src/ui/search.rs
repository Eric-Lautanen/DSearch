...
impl SearchPanel {
    pub fn ui(&mut self, ui: &mut egui::Ui, api: &ApiHelper) {
        ui.horizontal(|ui: &mut egui::Ui| {
            ...
            let enter_pressed = ui.input(|i: &egui::InputState| i.key_pressed(egui::Key::Enter)) && !self.query.is_empty();
            ...
            egui::ScrollArea::vertical().show(ui, |ui: &mut egui::Ui| {
                for result in &self.results {
                    ui.group(|ui: &mut egui::Ui| {
                        ui.horizontal(|ui: &mut egui::Ui| {
                            ui.label(egui::RichText::new(&result.schema).small().color(egui::Color32::from_rgb(0x64, 0xB5, 0xF6)));
                            ui.label(egui::RichText::new("•").small().color(egui::Color32::GRAY));
                            ui.label(egui::RichText::new(&result.source_url).small().color(egui::Color32::GRAY));
                        });
                        ui.add_space(2.0);
                        ui.label(egui::RichText::new(&result.title).strong());
                        ui.add_space(2.0);
                        ui.label(egui::RichText::new(&result.body_snippet).small().color(egui::Color32::LIGHT_GRAY));
                        ui.add_space(2.0);
                        ui.horizontal(|ui: &mut egui::Ui| {
                            ui.label(egui::RichText::new(format!("ID: {}…", &result.id[..12.min(result.id.len())]))
            ...
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&body) {
                if let Some(results) = v.get("results").and_then(|r| r.as_array()) {
                    for r in results {
                        ...
                        let schema = r.get("schema").and_then(|v| v.as_str()).unwrap_or("").to_string();
                        let source_url = r.get("source_url").and_then(|v| v.as_str()).unwrap_or("").to_string();
                        let body_text = r.get("body").and_then(|v| v.as_str()).unwrap_or("");
                        let snippet: String = body_text.chars().take(200).collect();
                        let title = r.get("tags")
